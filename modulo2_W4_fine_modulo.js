...
Il tuo compito è creare una funzione che cercherà per posizione lavorativa E posizione geografica. Questi due 
valori verranno passati come parametri
...
Cercando contemporaneamente come posizione lavorativa "dev" e posizione geografica "US", dovresti ottenere come 
risultato solamente job1 e job3,
...
function adsJobs (position, place){
  // metto i miei input in lowercase
  let dataTitle = position.toLowerCase();
  let dataLocation = place.toLowerCase();
  let result = [];
  // ciclo l'array
  for (let i = 0; i < jobs.length; i++) {
    // mi suddivido le parti dell'array e le porto il lowercase
    let theJob = jobs[i];
    let jobsTitleLowerCase = theJob.title.toLowerCase();
    let jobsLocationLowerase = theJob.location.toLowerCase();
    // paragono input e array
    if((jobsTitleLowerCase.includes(dataTitle)) && jobsLocationLowerase.includes(dataLocation)){
      // rimetto i risultati insieme e li mando in un array result
      let boxResult = { title: theJob.title, location: theJob.location}
      result.push(boxResult);
    }
